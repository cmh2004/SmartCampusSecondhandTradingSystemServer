...
bool DatabaseManager::addUser(const QJsonObject& user)
{
    ...
    QString sql = R"(
        INSERT INTO `user` (account, password, salt, nickname, phone, email, role, status, credit_score, register_time, school)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    )";
...
{
    QString sql = R"(
        INSERT INTO goods (seller_id, category_id, name, price, description, status, publish_time, school)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    )";
    QList<QVariant> binds;
    binds << goods.value("seller_id").toInt()
          ...
          << goods.value("status").toInt(0)
          << QDateTime::currentDateTime()
          << goods.value("school").toString();
    return execInsert(sql, binds);
}
...
QJsonArray DatabaseManager::searchGoods(const QString& keyword, int categoryId,
                                        ...
                                        const QString& sortBy, int page, int pageSize,const QString& school)
{
    QMutexLocker locker(&m_mutex);
...
QJsonArray DatabaseManager::getChatList(int userId)
{
    ...
    QString sql = R"(
        SELECT
            c1.session_id,
            MAX(c1.goods_id) as goods_id,
            COALESCE(MAX(g.name), '') as goods_name,
            MAX(c1.create_time) as last_time,
            (SELECT content FROM chat c2
             WHERE c2.session_id = c1.session_id
             ORDER BY create_time DESC LIMIT 1) as last_content,
            CASE
                WHEN MAX(c1.buyer_id) = ? THEN
                    (SELECT nickname FROM `user` WHERE id = MAX(c1.seller_id))
                ELSE
                    (SELECT nickname FROM `user` WHERE id = MAX(c1.buyer_id))
            END as other_name,
            CASE
                WHEN MAX(c1.buyer_id) = ? THEN MAX(c1.seller_id)
                ELSE MAX(c1.buyer_id)
            END as other_id
        FROM chat c1
        JOIN goods g ON c1.goods_id = g.id
        WHERE c1.buyer_id = ? OR c1.seller_id = ?
        GROUP BY c1.session_id
        ORDER BY last_time DESC
    )";
    return execSelectMany(sql, {userId, userId, userId, userId});
}
    ...
    QString sql = R"(
        SELECT
            ...
            CASE r.reported_type
                WHEN 1 THEN (SELECT nickname FROM user WHERE id = (SELECT seller_id FROM goods WHERE id = r.reported_id))
                WHEN 2 THEN (SELECT nickname FROM user WHERE id = r.reported_id)
                WHEN 3 THEN (SELECT nickname FROM user WHERE id = (SELECT seller_id FROM `order` WHERE id = r.reported_id))
                ELSE '未知'
...
QJsonArray DatabaseManager::getDisputes(int status, int page, int pageSize)
{
    ...
    QString sql = R"(
        SELECT d.*,
               uc.nickname AS complainant_name,
               ud.nickname AS defendant_name
        FROM dispute d
        LEFT JOIN `user` uc ON d.complainant_id = uc.id
        LEFT JOIN `user` ud ON d.defendant_id = ud.id
        WHERE 1=1
    )";
    QList<QVariant> binds;
    if (status >= 0) {
...
{
    QMutexLocker locker(&m_mutex);
    QString sql = "SELECT goods.*, u.nickname as seller_name, "
                  "(SELECT image_url FROM goods_image WHERE goods_id = goods.id ORDER BY sort_order LIMIT 1) AS image_url "
                  "FROM goods "
...
bool DatabaseManager::applyCreditChange(int userId, const QString& ruleCode, int relatedId, int adminId, int overrideChangeValue)
{
    QMutexLocker locker(&m_mutex);

    // 获取规则配置
    if (!CreditRules::instance()->isValidRule(ruleCode)) {
        qWarning() << "Invalid or disabled rule:" << ruleCode;
        return false;
    }
    CreditRule rule = CreditRules::instance()->getRule(ruleCode);
    int changeValue = (overrideChangeValue != 0) ? overrideChangeValue : rule.changeValue;

    // 检查月度限制
    if (rule.maxMonthly > 0) {
        int count = getMonthlyCreditChangeCount(userId, ruleCode);
        if (count >= rule.maxMonthly) {
            qInfo() << "User" << userId << "exceeded monthly limit for rule" << ruleCode;
            return false;
        }
    }
    ...
    int oldScore = user.value("credit_score").toInt(100);
    int newScore = qBound(0, oldScore + changeValue, 100);
    ...
    // 映射 change_type
    int changeType = 3; // 默认违规扣分
    if (ruleCode == "REPORT_VALID" || ruleCode == "NO_VIOLATION_30D") {
        changeType = 1;   // 加分奖励类
    } else if (ruleCode == "ORDER_TIMEOUT" || ruleCode == "DISPUTE_SELLER" ||
               ruleCode == "DISPUTE_BUYER" || ruleCode == "REPORT_GOODS" || ruleCode == "REPORT_USER") {
        changeType = 3;
    }
    ...
    query.prepare(R"(
        ...
        (user_id, rule_code, change_value, current_score, reason, related_id, admin_id, change_type, create_time)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    )");
    QString reason = rule.name;
    if (overrideChangeValue != 0 && ruleCode == "DISPUTE_SELLER") {
        reason = QString("纠纷判责卖家责任，扣除%1分").arg(qAbs(overrideChangeValue));
    } else if (overrideChangeValue != 0 && ruleCode == "DISPUTE_BUYER") {
        reason = QString("纠纷判责买家责任，扣除%1分").arg(qAbs(overrideChangeValue));
    }
    query.addBindValue(userId);
    query.addBindValue(ruleCode);
    query.addBindValue(changeValue);
    query.addBindValue(newScore);
    query.addBindValue(reason);
    query.addBindValue(relatedId);
    ...
    // 发送系统消息
    addSystemMessage(userId, 2, "信用分变更通知",
                     QString("您的信用分%1%2分，当前信用分：%3，原因：%4")
                         .arg(changeValue > 0 ? "增加" : "扣除")
                         .arg(qAbs(changeValue))
                         .arg(newScore)
                         .arg(reason),
                     0);
    return true;
...
int DatabaseManager::getMonthlyCreditChangeCount(int userId, const QString& ruleCode)
{
    QSqlQuery query(m_db);
    query.prepare("SELECT COUNT(*) FROM credit_record "
...
QJsonArray DatabaseManager::getUserInteractedGoods(int userId)
{
    QMutexLocker locker(&m_mutex);
    QString sql = "SELECT goods_id FROM favorite WHERE user_id = ? "
                  "UNION "
                  "SELECT goods_id FROM `order` WHERE buyer_id = ? AND status = 2";
    return execSelectMany(sql, {userId, userId});
}

QJsonArray DatabaseManager::getCollaborativeRecommendations(int userId, int limit, const QString& school)
{
    // 1. 获取用户交互过的商品ID（收藏或已购买）
    QJsonArray interacted = getUserInteractedGoods(userId);
    if (interacted.isEmpty()) {
        return QJsonArray(); // 无交互，返回空，让调用方降级到热度推荐
    }

    // 2. 构造排除商品ID列表的 IN 子句
    QStringList ids;
    for (const QJsonValue &val : interacted) {
        ids << val.toString();
    }
    QString idList = "(" + ids.join(",") + ")";

    // 3. 根据学校是否为空，选择不同的 SQL
    if (school.isEmpty()) {
        // 情况 A：用户未设置学校 → 只按共同行为人数排序
        QString sql = QString(R"(
            WITH user_behaviors AS (
                SELECT user_id, goods_id FROM favorite
                UNION
                SELECT buyer_id, goods_id FROM `order` WHERE status = 2
            )
            SELECT
                g.id,
                g.name,
                g.price,
                (SELECT image_url FROM goods_image WHERE goods_id = g.id ORDER BY sort_order LIMIT 1) AS image_url,
                COUNT(DISTINCT ub2.user_id) AS common_users
            FROM user_behaviors ub1
            JOIN user_behaviors ub2 ON ub1.goods_id = ub2.goods_id AND ub2.user_id != ?
            JOIN goods g ON ub2.goods_id = g.id
            WHERE ub1.user_id = ?
              AND g.status = 1
              AND g.id NOT IN %1
            GROUP BY g.id
        ...
        return execSelectMany(sql, {userId, userId, limit});
    } else {
        // 情况 B：用户已设置学校 → 同校商品优先排序
        QString sql = QString(R"(
            WITH user_behaviors AS (
                SELECT user_id, goods_id FROM favorite
                UNION
            ...
            SELECT
                ...
                (SELECT image_url FROM goods_image WHERE goods_id = g.id ORDER BY sort_order LIMIT 1) AS image_url,
                COUNT(DISTINCT ub2.user_id) AS common_users,
                CASE WHEN g.school = ? THEN 1 ELSE 0 END AS same_school
            FROM user_behaviors ub1
            JOIN user_behaviors ub2 ON ub1.goods_id = ub2.goods_id AND ub2.user_id != ?
            JOIN goods g ON ub2.goods_id = g.id
            WHERE ub1.user_id = ?
              ...
              AND g.id NOT IN %1
            GROUP BY g.id
            ORDER BY same_school DESC, common_users DESC
            LIMIT ?
        )").arg(idList);
        return execSelectMany(sql, {school, userId, userId, limit});
    }
}

QJsonArray DatabaseManager::getHotRecommendations(int limit)
{
    QMutexLocker locker(&m_mutex);
    QString sql = R"(
        SELECT
            ...
            g.price,
            (SELECT image_url FROM goods_image WHERE goods_id = g.id ORDER BY sort_order LIMIT 1) AS image_url,
            (g.view_count + g.favorite_count * 10) AS hot_score
        FROM goods g
        WHERE g.status = 1
        ORDER BY hot_score DESC
        LIMIT ?
    )";
    return execSelectMany(sql, {limit});
}


QString DatabaseManager::getUserEmailByAccount(const QString& account)
{
    QMutexLocker locker(&m_mutex);
    QSqlQuery query(m_db);
    query.prepare("SELECT email FROM `user` WHERE account = ?");
...
QString DatabaseManager::getMaskedEmail(const QString& email)
{
    if (email.isEmpty()) return QString();
    int atPos = email.indexOf('@');
    if (atPos <= 2) return "***" + email.mid(atPos);
    QString masked = email.left(2) + "****" + email.mid(atPos);
    return masked;
}

bool DatabaseManager::hasActiveDispute(int orderId)
{
    QSqlQuery query(m_db);
    query.prepare("SELECT 1 FROM dispute WHERE order_id = ? AND status != 3 LIMIT 1");
    query.addBindValue(orderId);
    return query.exec() && query.next();
}

bool DatabaseManager::hasResolvedDispute(int orderId)
{
    QSqlQuery query(m_db);
    query.prepare("SELECT 1 FROM dispute WHERE order_id = ? AND status = 2 LIMIT 1");
    query.addBindValue(orderId);
    return query.exec() && query.next();
}

int DatabaseManager::getUserCount(const QString& role, const QString& status, const QString& keyword)
{
    QString sql = "SELECT COUNT(*) FROM `user` WHERE 1=1";
    QList<QVariant> binds;
    if (!role.isEmpty()) { sql += " AND role = ?"; binds << role.toInt(); }
    if (!status.isEmpty()) { sql += " AND status = ?"; binds << status.toInt(); }
    if (!keyword.isEmpty()) { sql += " AND account LIKE ?"; binds << ("%" + keyword + "%"); }
    QSqlQuery query(m_db);
    query.prepare(sql);
...
int DatabaseManager::getPendingGoodsCount()
{
    QSqlQuery query(m_db);
    query.prepare("SELECT COUNT(*) FROM goods WHERE status = 0");
    if (query.exec() && query.next()) return query.value(0).toInt();
    return 0;
}

int DatabaseManager::getDisputeCount(const QString& status)
{
    QString sql = "SELECT COUNT(*) FROM dispute WHERE 1=1";
    QList<QVariant> binds;
    if (!status.isEmpty()) {
        sql += " AND status = ?";
        binds << status.toInt();
...
int DatabaseManager::getReportCount(const QString& status)
{
    QString sql = "SELECT COUNT(*) FROM report WHERE 1=1";
    QList<QVariant> binds;
    if (!status.isEmpty()) {
        sql += " AND status = ?";
        binds << status.toInt();
...
int DatabaseManager::getGoodsForReviewCount(const QString &keyword, int status,
                                            const QString &startDate, const QString &endDate)
{
    QString sql = "SELECT COUNT(*) FROM goods WHERE 1=1";
    QList<QVariant> binds;
    ...
    qCritical() << "getGoodsForReviewCount failed:" << query.lastError().text()
                << "SQL:" << sql;
    return -1;   // 查询失败返回 -1
}

int DatabaseManager::getGoodsCount(const QString& keyword, int categoryId,
                                   double minPrice, double maxPrice,
                                   const QString& school) {
    QMutexLocker locker(&m_mutex);
    QString sql = "SELECT COUNT(*) FROM goods WHERE status = 1";
    QList<QVariant> binds;
...
int DatabaseManager::getGoodsCountBySeller(int sellerId, int status) {
    QMutexLocker locker(&m_mutex);
    QString sql = "SELECT COUNT(*) FROM goods WHERE seller_id = ?";
    QList<QVariant> binds;
    binds << sellerId;
    if (status >= 0) {
        sql += " AND status = ?";
...
int DatabaseManager::getOrdersCount(int userId, int status, const QString& keyword) {
    QMutexLocker locker(&m_mutex);
    QString sql = "SELECT COUNT(*) FROM `order` WHERE (buyer_id = ? OR seller_id = ?)";
    QList<QVariant> binds;
    binds << userId << userId;
    if (status >= 0) {
        sql += " AND status = ?";
...
int DatabaseManager::getFavoritesCount(int userId) {
    QMutexLocker locker(&m_mutex);
    QSqlQuery query(m_db);
    query.prepare("SELECT COUNT(*) FROM favorite WHERE user_id = ?");
    query.addBindValue(userId);
    if (query.exec() && query.next())
        return query.value(0).toInt();
    return 0;
...
int DatabaseManager::getMyReviewsCount(int userId) {
    QMutexLocker locker(&m_mutex);
    QSqlQuery query(m_db);
    query.prepare("SELECT COUNT(*) FROM evaluation WHERE buyer_id = ?");
    query.addBindValue(userId);
    if (query.exec() && query.next())
        return query.value(0).toInt();
    return 0;
...
int DatabaseManager::getBrowseHistoryCount(int userId) {
    QMutexLocker locker(&m_mutex);
    QSqlQuery query(m_db);
    query.prepare("SELECT COUNT(*) FROM browse_history WHERE user_id = ?");
    query.addBindValue(userId);
    if (query.exec() && query.next())
        return query.value(0).toInt();
    return 0;
...
int DatabaseManager::getCreditHistoryCount(int userId) {
    QMutexLocker locker(&m_mutex);
    QSqlQuery query(m_db);
    query.prepare("SELECT COUNT(*) FROM credit_record WHERE user_id = ?");
    query.addBindValue(userId);
    if (query.exec() && query.next())
        return query.value(0).toInt();
    return 0;
...
int DatabaseManager::getSystemMessagesCount(int userId, bool unreadOnly) {
    QMutexLocker locker(&m_mutex);
    QString sql = "SELECT COUNT(*) FROM system_message WHERE user_id = ?";
    QList<QVariant> binds;
    binds << userId;
    if (unreadOnly) {
        sql += " AND is_read = 0";
